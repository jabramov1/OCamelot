(** Given an integer [n] and a list of items [l], produce a list containing
    elements of the list [l] up to index [n]. If [n] is bigger than [l], return
    the whole list. If [n < 0], return the empty list. *)
...
(** Given a CSV data representation [data], extract the closing prices and
    ...
    [divide_windows]. *)
let price_windows data n = CsvReader.get_closing_prices data |> divide_windows n

(** Given a CSV data representation [data], extract the volumes and divide them
    into a list of all contiguous sublists of length [n] using [divide_windows]. *)
...
let single_sma window =
  ...
  if List.length valid_prices = 0 then None
    ...
    let sma = sum valid_prices /. (List.length valid_prices |> float_of_int) in
    Some sma
...
(** [single_ema] calculates the exponential moving average for a singular window
    of time.
    ...
    @param multiplier
      Smoothing constant calculated by 2 / (n + 1), where n is the length of the
      window. *)
...
let exp_moving_avg data n =
  ...
  if n <= 0 then []
    ...
    else calculate_ema windows (Option.value init_ema ~default:0.) multiplier []
