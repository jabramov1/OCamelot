...
module OwlGraph : OwlGraphType = struct
  let input_csv (filename : string) : string list list =
    let result = Csv_reader.CsvReader.read_csv filename
  ...
  let gen_range acc stop num_days =
  ...
  let fifty_day_graph (data : string list list) =
    let y2 = Mat.uniform 10 100 in
    let h = Plot.create ~m:1 ~n:2 "" in
    Plot.(boxplot ~h ~spec:[ RGB (0, 153, 51) ] y2);
    Plot.output h

  let overlay_graph (data : string list list) =
    let y2 = Mat.uniform 10 100 in
    let h = Plot.create ~m:1 ~n:2 "" in
    Plot.(boxplot ~h ~spec:[ RGB (0, 153, 51) ] y2);
    Plot.output h
