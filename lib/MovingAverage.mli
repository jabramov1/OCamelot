...
(** [simple_moving_avg data n] calculates the simple moving average (SMA) for
    ...
    Reference: https://www.fmlabs.com/reference/default.htm
    ...
    @return
      A list of optional float values representing the SMAs calculated for each
      consecutive window in the dataset. If [n] is less than or equal to 0, the
      empty list is returned. If [n] is greater than the number of closing
      prices, then the [n] will be reassigned to a value equal to the number of
      closing prices.
...
(** [exp_moving_avg data n] calculates the exponential moving average (EMA) for
    the closing prices in a CSV data representation [data] over a window size
    [n].
    ...
    @return
      ...
      consecutive windows in the dataset. If [n] is less than or equal to 0, the
      empty list is returned. If [n] is greater than the number of closing
      prices, then the [n] will be reassigned to a value equal to the number of
      closing prices.
...
(** [vol_adj_moving_avg data n] calculates the volume-adjusted moving average
    ...
    @return
      A list of optional float values representing the VAMA calculated for each
      consecutive window in the dataset. If [n] is less than or equal to 0, the
      empty list is returned. If [n] is greater than the number of closing
      prices, then the [n] will be reassigned to a value equal to the number of
      closing prices.
